...
These environment variables should also be configured on Heroku - see (below)[https://github.com/theodi/pathway#deployment-on-heroku] #TODO. The Mandrill ones (and those for Postgres which are not shown here) are handled when the addon is deployed for the app. The others needs to be manually added.
